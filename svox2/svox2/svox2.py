...
class SparseGrid(nn.Module):
    ...
    # custom
    def apply_expanding_bbox(self, Pmin_init: np.ndarray, Pmax_init: np.ndarray, curr_it: int, max_it: int):
        """
        Sets gradients outside expanidng bbox to zero.
        BBox expands from Pmin_init, Pmax_init to full size.

        :param Pmin_init: (1,3) initial min value of bbox for each axis from 0.0 to 1.0
        :param Pmax_init: (1,3) initial max value of bbox for each axis from 0.0 to 1.0
        :param curr_it: int, current training iteration
        :param max_it: int, iteration the bounding is finished expanding
        """
        if curr_it > max_it:
            return
        ...
        axis0_indices = self.links[np.arange(-(N[0]-Pmax[0]),Pmin[0]),:,:][self.links[np.arange(-(N[0]-Pmax[0]),Pmin[0]),:,:] > -1].long()
        axis1_indices = self.links[:,np.arange(-(N[1]-Pmax[1]),Pmin[1]),:][self.links[:,np.arange(-(N[1]-Pmax[1]),Pmin[1]),:] > -1].long()
        axis2_indices = self.links[:,:,np.arange(-(N[2]-Pmax[2]),Pmin[2])][self.links[:,:,np.arange(-(N[2]-Pmax[2]),Pmin[2])] > -1].long()

        self.density_data.grad[axis0_indices] = 0
        self.density_data.grad[axis1_indices] = 0
        self.density_data.grad[axis2_indices] = 0

        self.sh_data.grad[axis0_indices] = 0
        self.sh_data.grad[axis1_indices] = 0
        self.sh_data.grad[axis2_indices] = 0
